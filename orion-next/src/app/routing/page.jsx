...
export default function RoutingPage() {
  ...
  const calculateRoute = async () => {
    ...
    setRouteData(null);
    try {
      const res = await fetch(`${API_URL}/api/routing?lat=${latitude}&lon=${longitude}`);
          ...
          <div className="md:col-span-5 bg-surface/60 backdrop-blur-xl border border-slate-700/50 rounded-2xl p-6 shadow-xl h-fit">
            ...
            <div className="flex flex-col gap-4">
              <div>
                ...
                <div className="w-full bg-void border border-slate-700 rounded-lg px-4 py-2 text-white font-mono">
                  {latitude.toFixed(6)}
                </div>
                <p className="text-[9px] font-mono text-slate-500 mt-1">Using global location from Settings</p>
              </div>
              <div>
